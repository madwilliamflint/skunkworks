import sys
import json
from PyQt5.QtWidgets import QApplication, QGraphicsScene, QGraphicsView, QMainWindow, QStatusBar, QMenuBar, QMenu, QAction, QFileDialog, QGraphicsItem, QGraphicsEllipseItem, QGraphicsLineItem, QDialog, QVBoxLayout, QLabel, QLineEdit, QPushButton, QColorDialog, QGraphicsRectItem
...
class RectNode(QGraphicsRectItem):
    ...
    def __init__(self, x, y, text, color="lightblue"):
        super().__init__(-30, -30, 60, 60)
        self.id = RectNode.node_counter
        RectNode.node_counter += 1
        self.setBrush(QBrush(QColor(color)))
        self.setPos(x, y)
        self.text = text
        self.color = color
    ...
    def mouseDoubleClickEvent(self, event):
        dialog = EditDialog(self)
        if dialog.exec_():
            self.color = self.brush().color().name()
            self.update()
