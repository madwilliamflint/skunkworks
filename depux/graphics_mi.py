...
# "multiple node types with common ancestor.

import sys
from PyQt5.QtWidgets import QGraphicsEllipseItem, QGraphicsRectItem, QGraphicsLineItem, QDialog, QVBoxLayout, QLabel, QLineEdit, QPushButton, QColorDialog
...
class EditDialog(QDialog):
    ...
    def choose_color(self):
        ...
        if color.isValid():
            if isinstance(self.item, BaseNode):
...
class BaseNode(QGraphicsEllipseItem):
    ...
    def __init__(self, x, y, text, color="yellow"):
        super().__init__()
        self.id = BaseNode.node_counter
        BaseNode.node_counter += 1
...
class RectNode(BaseNode, QGraphicsRectItem):
    def __init__(self, x, y, text, color="yellow"):
        super().__init__(x, y, text, color)
        self.setRect(-30, -30, 60, 60)
